...
function Doctors() {
  ...
  const [speciality, setSpeciality] = useState(routeSpeciality || '');
    ...
    <div className="w-full flex flex-col justify-center items-center gap-3 py-7 px-4 text-gray-800">
      ...
      <div className="flex flex-col gap-6 mt-5 w-full max-w-5xl">
        ...
        <div className="flex flex-wrap gap-2 justify-center text-sm text-gray-600">
          {['All', 'General Physician', 'Gynecologist', 'Dermatologist', 'Pediatricians', 'Neurologist', 'Gastroenterologist'].map((specialty, index) => (
            <p
              key={index}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {filterDoctors.map((doctor) => (
            <div
              onClick={() => navigate(`/appointment/${doctor._id}`)} // Pass docId as doc1, doc2, etc.
              key={doctor._id}
